...
def fifo():
    ...
    global page_frames
    global fifo_faults
    fifo_q = Queue.Queue(REF_STR_SIZE)

    for i in range(REF_STR_SIZE):
        fifo_q.put(page_ref_str[i])  # copy page references to FIFO queue
    ...
    for ref in page_ref_str:         # iterate page reference list
        ...
        if not page_found:
            ...
            if not free_frame_found:  # page fault: need to replace a non-empty frame via FIFO
                fifo_faults += 1
                print 'Page fault! FIFO faults = ' + str(fifo_faults)  # debug
                frame_to_swap = fifo_q.get()  # pop frame to be swapped from FIFO queue
                print 'Next page in FIFO queue to swap is ' + str(frame_to_swap)
                for index in range(page_frames_amt):
                    if page_frames[index] == frame_to_swap:   # found frame to be swapped
                        print 'Replaced existing frame ' + str(index) + ' = ' + str(page_frames[index]) + '',  # debug
                        page_frames[index] = ref
                        print ' with page ' + str(ref)  # debug
